...
WebAudiox.AdaptativeNormalizer = function(factorForMin, factorForMax) {
  ...
  this.update = function(value) {
    ...
    var smoothIn = 0.01
    if (value < minThreshold) {
      minThreshold += (value - minThreshold) * smoothOut
    } else {
      minThreshold += (value - minThreshold) * smoothIn
    }
    if (value > maxThreshold) {
      maxThreshold += (value - maxThreshold) * smoothOut
    } else {
      maxThreshold += (value - maxThreshold) * smoothIn
    }
